...
namespace DhlVendor\Octolize\Onboarding\PluginUpgrade;

use DhlVendor\Octolize\Onboarding\Field\Html;
use DhlVendor\Octolize\Onboarding\Onboarding;
use DhlVendor\Octolize\Onboarding\OnboardingButton;
use DhlVendor\Octolize\Onboarding\OnboardingOption;
use DhlVendor\Octolize\Onboarding\OnboardingShouldShowAlwaysStrategy;
use DhlVendor\Octolize\Onboarding\OnboardingShouldShowStrategy;
use DhlVendor\Octolize\Onboarding\OnboardingStep;
use DhlVendor\Octolize\Onboarding\OnboardingTrackerData;
...
class PluginUpgradeOnboardingFactory
{
    ...
    private function prepare_steps(string $previous_version, string $current_version): array
    {
        ...
        $onboarding_step->set_show(\true)->set_heading(sprintf(__('Thank you for updating %1$s!', 'flexible-shipping-dhl-express'), $this->plugin_name))->set_sub_heading(__('It is really important to keep the plugins up to date. We have implemented some improvements and new functionalities. Find out what has changed:', 'flexible-shipping-dhl-express'));
    ...
    {
        return [(new OnboardingButton())->set_label(__('I\'m not interested', 'flexible-shipping-dhl-express'))->set_classes(OnboardingButton::BTN_LINK), (new OnboardingButton())->set_label(__('Thanks for letting me know', 'flexible-shipping-dhl-express'))->set_type(OnboardingButton::TYPE_CLOSE)];
    }
